...
import { useAISearchContext, type ChatUIMessage, type SearchTool } from './search';
...
function AISearchInputActions({ chat }: { chat: ReturnType<typeof useChat<ChatUIMessage>> }) {
...
function AISearchInput({
  chat,
  ...props
}: ComponentProps<'form'> & { chat: ReturnType<typeof useChat<ChatUIMessage>> }) {
  const { status, sendMessage, stop } = chat;
  const [input, setInput] = useState(() => localStorage.getItem(StorageKeyInput) ?? '');
  const isLoading = status === 'streaming' || status === 'submitted';
  const onStart = (e?: { preventDefault: () => void }) => {
    e?.preventDefault();
...
function AISearchPanelList({
  ...
  style,
  ...props
}: ComponentProps<'div'> & { chat: ReturnType<typeof useChat<ChatUIMessage>> }) {
...
export function ChatPanel() {
  const { open, setOpen, pending, setPending } = useAISearchContext();
  ...
  useEffect(() => {
    ...
    void chat.sendMessage({
      role: 'user',
      parts: [
        { type: 'data-client', data: { location: pending.location } },
        { type: 'text', text: pending.text },
      ],
    });
    // chat is a stable useChat() handle; re-running this on every render
    // would resend the same pending message.
